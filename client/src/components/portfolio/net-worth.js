...
function convertToValue(marketInfo, portfolio) {
    let vals = [];
    let net = 0;
    let investment = 0;
    ...
    for (const key in portfolio) {
        if (key !== 'USD' && key !== 'timestamp' && key !== 'datetime' && key !== 'USDT') {
            let value = marketInfo.RAW[key].USD.PRICE * portfolio[key];
            net += value;
            vals.push({
                currency: key,
...
const NetWorth = (props) => {
    if (props.portfolio.length === 0 || props.ticker.length === 0 || !props.marketInfo) {
        return <div></div>;
    }
    ...
    const net = convertToValue(props.marketInfo, holdings);
    return (
